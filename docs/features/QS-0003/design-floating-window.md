...
> 版本：1.1
> 日期：2026-06-11（2026-09-14 修订：点击不再展开，直接滑动展开，防桌面误触；长按把手蓄力后解锁拖拽，松手吸附最近边）
> 状态：设计待评审
...
为 QuickSave 增加一个**全局悬浮窗**：总开关开启后常驻于所有应用之上，平时贴边显示一个半透明小把手，向屏幕内侧滑动展开成一个固定尺寸、横排 2 按钮的小面板，提供两个预定义快捷操作：
...
- 常驻悬浮窗：贴边半透明把手、滑动展开/收起、长按后拖拽定位（松手吸附最近边）、操作后自动收回
- 横排固定尺寸 2 按钮面板
...
```
 ...
 [贴边收起] ───────────────► [展开面板]
     ...
     └──── 点空白处 / 操作完成 ─────┘
```
- **贴边收起**：半透明竖条把手，吸附在屏幕左/右边；点击不触发（防误触）；直接向内滑动 ≥48dp 展开；长按把手（放大蓄力）后拖动改位置/换边，松手吸附最近边。
